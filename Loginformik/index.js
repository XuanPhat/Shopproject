...
const ValidationSchema = Yup.object().shape({
  ...
  tu_ngay: Yup.mixed()
    .test('tu_ngay-valid', 'Ngày đến: Không hợp lệ', (val) =>
      moment(val, 'DD/MM/YYYY HH:mm:ss').isValid(),
    )
    .test(
      'tu_ngay-length',
      'Ngày đến: Không hợp lệ',
      (val) => val && val.length === 19,
    )
    .required('Ngày đến: Chưa nhập'),
  den_ngay: Yup.mixed()
    .test('den_ngay-valid', 'Ngày đi: Không hợp lệ', (val) =>
      moment(val, 'DD/MM/YYYY HH:mm:ss').isValid(),
    )
    .test(
      'den_ngay-length',
  ...
  ngay_sinh: Yup.mixed()
    .test('ngay_sinh-valid', 'Ngày sinh: Không hợp lệ', (val) =>
...
export default function Login() {
  ...
  React.useEffect(() => {
    formRef.current.zoomIn(900);
    const keyboardDidShowListener = Keyboard.addListener(
      'keyboardDidShow',
      () => {
        headerRef.current.transitionTo({height: 0}, 750);
    ...
    const keyboardDidHideListener = Keyboard.addListener(
      'keyboardDidHide',
      () => {
        headerRef.current.transitionTo({height: 250}, 750);
        // setHeight(240);
      },
    );
